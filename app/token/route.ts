import { unseal, seal, now, ACCESS_TTL, REFRESH_TTL, verifyPkceS256, type AuthCode, type AccessToken, type RefreshToken } from "@/lib/oauth";
...
function issueTokens(key: string): Response {
  const access_token = seal<AccessToken>({ t: "at", key, exp: now() + ACCESS_TTL });
  const refresh_token = seal<RefreshToken>({ t: "rt", key, exp: now() + REFRESH_TTL });
